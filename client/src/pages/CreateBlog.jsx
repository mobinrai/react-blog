import { Button, FormControlLabel, FormGroup, styled, TextField } from '@mui/material'
...
import { fetchAllCategory } from '../../queries/CategoryQuery';
import { createPost } from '../../queries/PostQuery';
...
const CreateBlog = () => {
    ...
    const {isPending, isError, data:categories, error} = fetchAllCategory()
    ...
    return (
        <section className="create-blog-post">
            <div className="max-w-6xl mx-auto px-2.5 my-8">
                ...
                <form onSubmit={handleSubmit} className='my-6 flex flex-col gap-6 w-[60%]'>
                    {
                    ...
                    <div className="flex flex-col gap-4">
                        ...
                        <select name="categoryId" id="" className='border p-3 rounded-md' required>
                        {
                            categories.map((item)=>(
                    ...
                    <div className='flex flex-row gap-1 flex-1 mb-10'>
                        ...
                        <ImageKitUpload type="video" 
                        onSuccess={setVideo}>
                            <VideoCall className='cursor-pointer'/>
                        </ImageKitUpload>
                        </div>
                        <ReactQuill theme='snow' className='flex-1' name="content" value={value} onChange={setValue} required/>
                    </div>
